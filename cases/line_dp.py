...
class Line(LineData, Model):
    ...
    def __init__(self, system=None, config=None):
       ...
       ## test (this (state) version is also in implicit form but has the same dq-axis alignment as in ANDES implementation)
        ...
        # test (this (algeb) version is also in implicit form but has the same dq-axis alignment as in ANDES implementation)
        self.idd = Algeb(info='real current',
                         tex_name='idd',
                         v_str='1e-8',
                         e_str='u * ( -(x+1e-8)*iqq - (r+1e-8)*idd - v2*sin(a2) + v1*sin(a1) ) + (1-u) * idd',
