...
function main()
    ...
    for line in lines
        ...
        # prob = MyDiffEq.ODEProblem(solve_dynamic_sim!, u0, (0.0, 5e-4*2), p, mass_matrix)
        try
            prob = MyDiffEq.ODEProblem(solve_dynamic_sim!, u0, (0.0, 5e-4*2), p, mass_matrix)
    ...
    @show lines = fail_line[fail_ind]
    # @show xs = fail_x[fail_ind]
    @show fail_t[fail_ind]

    return fail_line, fail_t
end

fail_line, fail_t = main()