...
function main()
    ...
    for x in x_fault_start:delta_x_fault:x_fault_end
        for bus in non_slack_buses
            ...
            # models.fault.x_fault[1] = 0.016
            models.fault.x_fault[1] = x
            ...
            # u1 = sol0.u[end]
            p = (address, sys.models, sys.incidence_matrix, sys.C_eq, sys.non_slack_buses, lambda)
            prob1 = MyDiffEq.ODEProblem(solve_dynamic_sim!, u0, (0.0, 5e-4*2), p, mass_matrix)
            try
            ...
            if sol1.retcode != :Success
                ...
                try
                    prob2 = MyDiffEq.ODEProblem(solve_dynamic_sim!, u0, (0.0, 5e-5*2), p, mass_matrix)
                ...
                if sol2.retcode != :Success
                    push!(fail_x, x)
                    push!(fail_bus, bus)
                    push!(fail_t, sol1.time[end])
                    continue
                end
            else
                success_count += 1
                # we do not need fault-off study
                continue
            end
    ...
    println("================================================")
    # return fail_x, fail_bus, fail_t

    fail_ind = findall(x-> x==0.0, fail_t)
    @show buses = fail_bus[fail_ind]
    @show xs = fail_x[fail_ind]

    return fail_x, fail_bus, fail_t
end

fail_x, fail_bus, fail_t = main()

fail_ind = findall(x-> x==0.0, fail_t)
buses = fail_bus[fail_ind]
xs = fail_x[fail_ind]
